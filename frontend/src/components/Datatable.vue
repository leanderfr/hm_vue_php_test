...
<template>
    ...
    <div class='flex flex-col h-full ' id='datatableContainer'>
      ...
      <div>...sajkas</div>
        ...
        <div class="flex flex-row pt-1">
            ...
            <!-- expressions table -->
            <div  class='btnEXPRESSIONS_TABLE putPrettierTooltip' :title="expressions.expressions" @click="emit('datatableToShow', 'expressions')" aria-hidden="true"></div>   

            <!-- schedule  -->
            <div  class='btnSCHEDULE_TABLE putPrettierTooltip' :title="expressions.expressions" @click="emit('displaySchedule')" aria-hidden="true"></div>   
    ...
    <div class="DatatableHeader" v-for='column in columns' :key="column"> 
...
const emit = defineEmits( ['showLoading', 'hideLoading','updateSelectedCar','datatableToShow','displaySchedule'] );
const props = defineProps( ['tableToDisplay', 'currentCountry', 'backendUrl', 'imagesUrl', 'expressions' ] )
...
async function fetchData() {
  ...
  await fetch(`${backendUrl.value}/${props.tableToDisplay}/${language}`)
